...
def list_message(youtube, broadcast_id):
  print('Messages from broadcast "%s":' % broadcast_id)
  ...
  while list_messages_request:
    list_messages_response = list_messages_request.execute()

    for message in list_messages_response.get('items', []):
      print('%s (%s)' % (message['snippet']['displayMessage'], message['authorDetails']['displayName']))

    list_messages_request = youtube.liveChatMessages().list_next(
      list_messages_request, list_messages_response)
...
if __name__ == '__main__':
  ...
  try:
    livechat_id = list_broadcasts(youtube, args.broadcast_status)
    list_message(youtube, livechat_id)
